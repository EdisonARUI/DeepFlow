...
const { buildTradeBootstrapTx } = await import("../src/trade/build-trade-bootstrap-tx.ts");
...
describe("buildTradeBootstrapTx (unit)", () => {
  const sender = "0xabc";
  ...
  it("runs supply → withdraw → swap → supply USDC in order (input-fee, deepAmount=0)", async () => {
    const tx = await buildTradeBootstrapTx({
      sender,
    ...
    expect(mockSwapExactQuantity.mock.invocationCallOrder[0]).toBeLessThan(
      mockDepositCoinPTB.mock.invocationCallOrder[1],
    );

    expect(mockDepositCoinPTB).toHaveBeenNthCalledWith(
      1,
      expect.any(Transaction),
      SUI_COIN_TYPE,
      suiCoinArg,
      expect.objectContaining({ amount: 10_000_000_000 }),
    );
  ...
  it("passes non-zero deepAmount for DEEP-fee swap branch", async () => {
    await buildTradeBootstrapTx({
      sender,
      suiAmount: 10_000_000_000n,
      minUsdcOut: 20_000_000n,
      deepAmount: 0.05,
