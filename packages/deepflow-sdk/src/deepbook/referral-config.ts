import { FEATURED_POOL_KEYS } from "./featured-pools.ts";
...
export function getRequiredReferralId(poolKey: string): string {
  ...
  if (!referralId) {
    throw new Error(
      `Missing DeepBook referral for pool ${poolKey}. Set ${INTEGRATION_REFERRAL_ENV_PREFIX}${poolKeyToEnvSuffix(poolKey)} or ${REFERRAL_ENV_PREFIX}${poolKeyToEnvSuffix(poolKey)}.`,
    );
  }
  return referralId;
}

export function listConfiguredReferralPoolKeys(): string[] {
