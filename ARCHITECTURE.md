...
| 目录 | 职责 | 状态 |
|------|------|------|
| `app/`、`components/`、`lib/` | 产品 Dashboard（Next.js App Router） | 四页 UI 已还原；Liquidity 读路径已接协议适配层（首期 NAVI） |
...
- `src/supply-withdraw.ts`：`simulateSupplyWithdraw` / `inspectSupplyWithdraw` 公开 API。

尚未创建 Trading 全链路真实 PTB Builder 与 DeepBook Adapter。Dashboard 已落地 NAVI **只读**协议适配器；SDK 已实现 NAVI supply/withdraw 写路径与 mainnet 模拟验证。
...
- Liquidity 页展示的 NAVI / Scallop / Cetus 为设计稿示意；**MVP 读路径仅启用 NAVI 适配器**，后续通过 `protocols/<name>/` 扩展。
...
LiquidityWorkspace
  ...
  -> NaviLiquidityAdapter          # @naviprotocol/lending（仅 Main Market + 标的白名单）
  -> mapToLiquidityViews()
```
...
```text
PositionManagement（后续接线）
  -> simulateSupplyWithdraw() / inspectSupplyWithdraw()   # @deepflow/sdk
  -> buildNaviSupplyTx / buildNaviWithdrawTx              # @naviprotocol/lending PTB
  -> dryRunTransaction / devInspectTransaction            # SuiGrpcClient.core.simulateTransaction
```
...
- 模拟不上链、不消耗资产；仍需真实 mainnet sender 地址与 coin objects（supply）或 NAVI supply 余额（withdraw）。
- 集成测试：`sdk/tests/navi-supply-withdraw.integration.test.ts`，门禁 `RUN_MAINNET_INTEGRATION=1` + `INTEGRATION_SENDER`。
...
| `NEXT_PUBLIC_LIQUIDITY_PROTOCOLS` | `navi` | 逗号分隔协议列表（未来 `navi,scallop`） |
...
deepflow/                          # 仓库根 = Next.js Dashboard
  ...
  lib/                             # 前端共享工具 [已有]
    ...
    data/                          # Dashboard 读路径数据层（Repository/Mapper/Hook）
      liquidity/
        ...
        protocols/                 # DeFi 协议只读适配器（首期 navi/）
          types.ts
  ...
  sdk/                             # @deepflow/sdk [已有]
    src/
      ...
      supply-withdraw.ts           # simulateSupplyWithdraw / inspectSupplyWithdraw
      sui/client.ts
