...
- 点击切换当前 Swap / Limit 目标池。

### DeepBook Orders（订单面板）

右侧 ORDERS 面板拆为上下两块（各占 50% 高度；不再与 DEEPBOOK 面板共用 SWAP/LIMIT 切换）：

- **Order History**（上半）：展示**全 featured 池**的 swap 与限价单 **FILLED / CANCELED / PARTIAL** 历史（不随左侧选中交易对过滤）；顶部 **ALL | SWAP | LIMIT** Tab 客户端筛选。
- **Open Limit Orders**（下半）：展示用户 BalanceManager 上**全池** **open / partial** 限价单；单笔 **Cancel** 使用订单自身 `poolKey`；**Cancel All** 对列表中所有有挂单的池依次 `cancelAllOrders`（execute 模式可能多次签名）。
- 限价单执行成功后，DEEPBOOK 面板提示 **「Limit order placed on book」** 或 **「Limit order filled immediately」**（解析 tx `OrderFullyFilled` / `order_inserted` 事件）。
- Live 模式需用户已有 DeepBook `BalanceManager`（首笔限价单 PTB 可自动 create + register + 绑定 Referral）。

### DeepBook Swaps（swap 成交历史）

- 已合并至 **Order History**（`kind: swap`）；仍通过 Sui RPC 解析用户交易中 `swap_exact_*` MoveCall。
- 未连接钱包时展示空状态说明；无记录时提示「No DeepBook orders yet」。

### Swap / Limit Widget（兑换与限价单）

- 顶部 **SWAP | LIMIT** 模式切换。
- **Swap 模式**：
  - **SOURCE / DESTINATION**：分段选择 `WALLET / NAVI / SUILEND`；PAY 余额按 SOURCE 展示；RECEIVE 余额按 DESTINATION 展示。
  - **Rate / Fee**：来自 DeepBook `getQuantityOutInputFee` 链上报价（mock 模式用 fixture 估算）。
  - **Execute**：按 SOURCE × DESTINATION 路由至不同真实 dry-run PTB（支持上述 7 个精选池的双向 swap）。
- **Limit 模式**：
  - 仅 **SOURCE**（无 DESTINATION）；资金 deposit 至 DeepBook BalanceManager 后 `place_limit_order`。
  - **SIDE**：BUY（deposit quote）/ SELL（deposit base）；**PRICE** + **QUANTITY**（base 数量）。
  - PTB：`withdraw/merge → balance_manager::deposit → pool::place_limit_order`（首单可同 PTB bootstrap BM + Referral 绑定）。
  - **Fee**：`getOrderDeepRequired` maker 估算；默认 `payWithDeep: false`（input-fee）。
  - 成交结算留在 BalanceManager；withdraw settled 为后续迭代。

#### DeepBook Referral（手续费返佣）

- DeepFlow 运维一次性为各 featured pool mint `DeepBookPoolReferral`（`scripts/mint-deepbook-referrals.mjs`）。
- 配置 env：`NEXT_PUBLIC_DEEPBOOK_REFERRAL_<POOLKEY>=0x...`（如 `NEXT_PUBLIC_DEEPBOOK_REFERRAL_SUI_USDC`）。
- 用户首笔限价单 bootstrap 时 `setBalanceManagerReferral` 绑定；后续该 BM 成交累积 referral 奖励至 DeepFlow 拥有者地址。
...
| 模式 | 说明 |
|------|------|
| `mock`（默认） | 使用 `lib/fixtures/trading.ts` 静态行情、swap 成交历史、open limit orders fixture；DeFi 余额仍来自 Liquidity mock fixture。 |
| `live` | DeepBook SDK 拉取 mid price / 报价 / 费率 / open orders；Sui RPC + Indexer 拉取 swap 成交历史；Liquidity live 路径提供 NAVI supplied 余额。 |
