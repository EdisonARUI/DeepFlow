...
- **Credit Source**：MVP 默认 NAVI；From 余额来自选中 DeFi 协议的 **supplied** 头寸（`useLiquidityPositions`）。
- **Rate / Fee**：来自 DeepBook `getQuantityOut` / `getQuantityOutInputFee` 链上报价（mock 模式用 fixture 估算）。
- **Execute**：触发 `simulateTrade`——策略校验 + RoutePlan + mock PTB；**MVP 仅 dry-run 模拟，不实际上链**。

### PTB Pipeline（执行管线）

- 底部实时展示构建中的 PTB 链上步骤：`WITHDRAW` → `DEEPBOOK_TRADE` → `SETTLE` → `REDEPOSIT`。
- 步骤状态：`pending` / `active` / `done` / `error`，随模拟进度更新。
