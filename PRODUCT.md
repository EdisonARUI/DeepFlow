...
- Treemap 展示用户在 **NAVI / SCALLOP / CETUS / DEEPBOOK / WALLET** 五类资金桶的 USD 分布；Treemap 内标签字体统一为黑色。
...
- MVP 精选池：`SUI_USDC`、`DEEP_SUI`、`WUSDT_USDC`、`WAL_USDC`。
- 点击切换当前 Swap 目标池。

### DeepBook Orders（历史订单）

- 展示用户 Balance Manager 的 **历史订单**（Placed / Filled / Canceled），非 order book 深度。
- Live 模式：SDK 发现用户 Balance Manager + DeepBook Indexer REST 聚合。
- 未连接钱包或无 Balance Manager 时展示空状态说明。
...
- **Execute**：按 SOURCE × DESTINATION 路由至不同真实 dry-run PTB（`SUI_USDC` 卖 SUI 换 USDC）；成功时展示「模拟通过」；**MVP 仅模拟，不实际上链**。

| Source | Destination | 行为 |
|--------|-------------|------|
| WALLET | WALLET | 钱包 SUI → DeepBook swap → 钱包 USDC |
| WALLET | NAVI | supply SUI → withdraw → swap → supply USDC（bootstrap） |
| NAVI | NAVI | withdraw SUI → swap → supply USDC |
| NAVI | WALLET | withdraw SUI → swap → 钱包 USDC |
| 含 SUILEND | 任意 | UI 可选；Execute 提示「Suilend 交易路径尚未支持」 |

### 数据模式

| 模式 | 说明 |
|------|------|
| `mock`（默认） | 使用 `lib/fixtures/trading.ts` 静态行情与历史订单；DeFi 余额仍来自 Liquidity mock fixture。 |
| `live` | DeepBook SDK 拉取 mid price / 报价 / 费率；Indexer 拉取历史订单；Liquidity live 路径提供 NAVI supplied 余额。 |

环境变量复用 `NEXT_PUBLIC_DATA_SOURCE`（与 Portfolio / Liquidity 一致）。
